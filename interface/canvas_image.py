...
class CanvasImage:
    def __init__(self, placeholder, path):
        ...
        self.canvas.bind('<ButtonPress-1>', self.__move_from)  # lembra da posição do canvas
        self.canvas.bind('<B1-Motion>',     self.__move_to)  # movimenta o canvas
        self.canvas.bind('<MouseWheel>', self.__wheel)  # zoom para windows e mac
        self.canvas.bind('<Button-5>',   self.__wheel)  # zoom para no Linux, scroll down
        self.canvas.bind('<Button-4>',   self.__wheel)  # zoom zoom no Linux, wheel scroll up
